...
function ghostWarnings(memoryContent, changedFiles) {
  ...
  for (const line of lines) {
    for (const file of changedFiles) {
      ...
      if (line.includes(file) || line.includes(basename3)) {
...
function autoGenerateSkills(memoryContent, workspace) {
  ...
  for (const [, v] of counts) {
    ...
    const basename3 = path2.basename(v.file, path2.extname(v.file));
    const skillName = `${v.category}-${basename3}`;
...
${ghostContent}`;
  ...
  if (!validation.valid) {
    core3.warning(`Defense anomaly detected: ${validation.anomalies.join("; ")}`);
  }
...
Return the filtered list with the same schema.`;
  try {
    ...
    core4.warning(`Critique LLM call failed: ${e instanceof Error ? e.message : String(e)} \u2014 falling back to confidence filter`);
    return filterByConfidence(review, config.confidenceThreshold);
  }
}
...
Is this issue real and actionable?`,
      ...
      core5.warning(`Calibration failed for ${finding.file}:${finding.line}: ${e instanceof Error ? e.message : String(e)}`);
  ...
  core5.info(`Confidence calibration: ${highCount} high, ${result.length - highCount - lowCount} medium, ${lowCount} low`);
  return result;
}
...
... Too many findings to display. (${review.comments.length} findings, body truncated to ${MAX_COMMENT_BODY} chars)`;
      core6.warning(`Review body truncated from ${originalLen} to ${MAX_COMMENT_BODY} chars`);
    }
    ...
    if (error2?.status === 422) {
      core6.warning("422 on createReview \u2014 falling back to summary-only comment");
...
function appendSpendEntry(workspace, entry) {
  ...
  try {
    ...
    core7.info(`Spend: ${entry.totalTokens} tokens (${entry.provider}/${entry.model})`);
    truncateIfNeeded(filePath);
  } catch (error2) {
    core7.warning(`Failed to write spend entry: ${error2}`);
  }
}
function truncateIfNeeded(filePath) {
  try {
    ...
    core7.warning(`Spend log rotation failed: ${e instanceof Error ? e.message : String(e)}`);
  }
}
...
function recordSuggestion(workspace, repo, file, line, category, severity, message) {
  ...
  try {
    ...
    core8.info(`Feedback: recorded suggestion for ${file}:${line} [${category}]`);
  } catch (e) {
    core8.warning(`Failed to record suggestion: ${e instanceof Error ? e.message : String(e)}`);
  } finally {
    db.close();
  }
}
function getCategoryStats(workspace, repo) {
  ...
  try {
    ...
    core8.warning(`Failed to get category stats: ${e instanceof Error ? e.message : String(e)}`);
    return [];
  } finally {
    db.close();
  }
...
function writeFeedbackStore(workspace, store) {
  ...
  try {
    ...
    core9.info(`Feedback: wrote ${store.entries.length} entries`);
  } catch (error2) {
    core9.warning(`Failed to write feedback: ${error2}`);
  }
}
...
async function applyFileFixes(octokit, owner, repo, headRef, byFile) {
  ...
  for (const [filePath, suggestions] of byFile) {
    if (isDangerousPath(filePath)) {
      core10.warning(`Skipping suspicious path: ${filePath}`);
      continue;
    }
    const entry = tree.tree.find((e) => e.path === filePath && e.type === "blob");
    if (!entry?.sha) {
      core10.warning(`Skipping ${filePath}: not found in tree`);
      continue;
    }
    ...
    for (const s of [...suggestions].sort((a, b) => b.line - a.line)) {
      ...
      if (idx >= 0 && idx < lines.length) {
        ...
        if (!verification.valid) {
          core10.warning(`Skipping invalid patch at ${filePath}:${s.line}: ${verification.reason}`);
          continue;
        }
        lines[idx] = s.code;
        fixedCount++;
...
async function generateFix(octokit, owner, repo, prNumber, _config) {
  ...
  core10.info(`Applied ${fixedCount} suggestion(s): ${nc.sha}`);
  ...
  const read_file = tool({
    ...
    execute: async ({ path: path11 }) => {
      if (isBlockedPath(path11)) {
        core11.warning(`Agent read_file blocked: ${path11} matches secret file pattern`);
...
${diffContent.slice(0, 15e3)}`;
  try {
    ...
    if (text) {
      core11.info(`Agent context: gathered ${text.length} chars of cross-file context`);
      return truncate(text, 2e3);
    }
    return "";
  } catch (e) {
    core11.warning(`Agent context gathering failed: ${e instanceof Error ? e.message : String(e)} \u2014 continuing without agent context`);
    return "";
  }
}
...
function runLinters(workspace, changedFiles) {
  ...
  try {
    ...
    core12.debug("ESLint scan skipped: " + (e instanceof Error ? e.message : String(e)));
  }
  try {
    const tscResults = runTsc(workspace);
    findings.push(...tscResults);
  } catch (e) {
    core12.debug("tsc scan skipped: " + (e instanceof Error ? e.message : String(e)));
  }
  try {
    const prettierResults = runPrettier(workspace, jsFiles);
    findings.push(...prettierResults);
  } catch (e) {
    core12.debug("Prettier scan skipped: " + (e instanceof Error ? e.message : String(e)));
  }
  if (findings.length > 0) {
    core12.info(`Linter pre-scan: ${findings.length} finding(s) from linters`);
  }
  return findings;
}
...
function runDependencyAudit(workspace) {
  ...
  try {
    ...
    core12.debug("npm audit skipped: " + (e instanceof Error ? e.message : String(e)));
  }
  try {
    const pipFindings = runPipAudit(workspace);
    findings.push(...pipFindings);
  } catch (e) {
    core12.debug("pip-audit skipped: " + (e instanceof Error ? e.message : String(e)));
  }
  if (findings.length > 0) {
    core12.info(`Dependency audit: ${findings.length} CVE finding(s)`);
  }
  return findings;
}
...
function runPipAudit(workspace) {
  ...
  try {
    ...
    core12.debug("pip-audit not available: " + (e?.message || String(e)));
  }
  return findings;
}
...
async function ensureLabel(octokit, owner, repo, def) {
  try {
    ...
    try {
      ...
      core13.debug(`Label '${def.name}' already exists or cannot be created`);
    }
  }
}
async function applyLabels(octokit, owner, repo, prNumber, findings, riskScore) {
  ...
  if (toAdd.length > 0 || toRemove.length > 0) {
    core13.info(`Auto-labels: +${toAdd.join(",")} -${toRemove.join(",")}`);
  }
...
var RateLimiter = class {
  ...
  async waitForToken(bucket, name) {
    ...
    if (waitMs > 0) {
      core14.debug(`Rate limit: waiting ${waitMs}ms for ${name} token`);
      await sleep(waitMs);
    }
    this.refill(bucket);
...
function createRateLimiter(provider) {
  ...
  const rpm = parseInt(core14.getInput("rpm") || "0", 10) || defaults.rpm;
  const rps = parseInt(core14.getInput("rps") || "0", 10) || defaults.rps;
  core14.info(`Rate limiter: ${provider} \u2014 ${rpm} RPM, ${rps} RPS`);
...
async function checkCompliance(octokit, owner, repo, _prNumber, prBody, prTitle, diffSummary, config) {
  ...
  for (const issueNum of issueRefs.slice(0, 3)) {
    try {
      ...
      core15.info(`Compliance: #${issueNum} \u2192 ${compliance.level} \u2014 ${compliance.summary}`);
    } catch {
      core15.warning(`Failed to fetch issue #${issueNum} for compliance check`);
    }
  }
  return results;
}
...
Does this PR implement the issue requirements?`;
  try {
    ...
    core15.warning(`Compliance evaluation failed: ${e instanceof Error ? e.message : String(e)}`);
...
async function processReactionApprovals(octokit, owner, repo, prNumber, config) {
  const token = process.env.GITHUB_TOKEN || core16.getInput("github_token");
  if (!token) return 0;
  const { data: pr } = await octokit.rest.pulls.get({ owner, repo, pull_number: prNumber });
  void pr;
  ...
  for (const comment of mizumiComments) {
    try {
      ...
      core16.info(`Found \u{1F44D} on comment ${comment.id} in ${comment.path} \u2014 auto-applying suggestion`);
      ...
      core16.warning(`Failed to process reaction on comment ${comment.id}: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  return applied;
}
...
async function persistLearningData(octokit, owner, repo, defaultBranch, workspace) {
  ...
  if (filesToCommit.length === 0) {
    core17.info("No learning data files to persist");
  ...
  try {
    ...
    core17.info(`Persisted ${filesToCommit.length} learning data file(s): ${newCommit.sha}`);
    return { committed: true, filesPushed: filesToCommit.length, commitSha: newCommit.sha };
  } catch (error2) {
    const msg = error2 instanceof Error ? error2.message : String(error2);
    core17.warning(`Failed to persist learning data: ${msg}`);
    return { committed: false, filesPushed: 0, commitSha: null };
  }
}
...
async function postPendingGate(octokit, owner, repo, headSha, prNumber) {
  try {
    ...
    core18.warning(`Failed to post pending gate status: ${e instanceof Error ? e.message : String(e)}`);
  }
}
async function postGateStatus(input) {
  ...
  try {
    ...
    core18.info(`Gate status: ${state} (threshold=${threshold}, findings=${findingCount})`);
  } catch (e) {
    core18.warning(`Failed to post gate status: ${e instanceof Error ? e.message : String(e)}`);
  }
  core18.setOutput("gate_status", state);
  return state;
}
...
import * as core19 from "@actions/core";
...
function loadCustomRules(workspace) {
  ...
  try {
    ...
    return parsed.rules.filter((r) => r.name && (r.pattern || r.file_glob)).map((r, i) => ({
      id: `custom-${i}`,
      name: String(r.name),
      description: String(r.description || r.name),
      source: "custom",
      type: r.type === "glob" ? "glob" : "regex",
      pattern: String(r.pattern),
      fileGlob: r.file_glob ? String(r.file_glob) : void 0,
    ...
    core19.warning(`Failed to load custom rules: ${e instanceof Error ? e.message : String(e)}`);
    return [];
  }
}
var DB_FILENAME2 = "mizumi-data.db";
...
function discoverRules(workspace, repo) {
  ...
  try {
    ...
    core19.warning(`Rule discovery failed: ${e instanceof Error ? e.message : String(e)}`);
    return [];
  } finally {
    db?.close();
  }
}
function loadDiscoveredRulesFromDb(db) {
  const query = db.prepare(`SELECT * FROM discovered_rules WHERE enabled = 1`);
  const rows = query.all();
  return rows.map(rowToPersistedRule);
}
function minePatternsFromDb(db, repo) {
  const query = db.prepare(`
    ...
    LIMIT 20
  `);
  const rows = query.all(repo, DISCOVERY_MIN_OCCURRENCES, DISCOVERY_MIN_ACCEPTANCE);
  return rows.map((r) => {
    ...
    const basename3 = path10.basename(r.file, ext);
    return {
      id: ruleId,
      name: `${r.category}-pattern-${basename3}`,
...
function mergeDiscoveredRules(existing, mined, db) {
...
var DECAY_MIN_REVIEWS = 20;
var DECAY_LOW_ACCEPTANCE = 0.3;
var DECAY_RATE = 5;
function applyRuleDecay(rules, workspace, repo) {
  ...
  try {
    ...
    const updated = rules.map((rule) => {
      ...
      const lastMatch = rule.lastMatchedAt ? new Date(rule.lastMatchedAt).getTime() : new Date(rule.createdAt).getTime();
      const daysSinceMatch = Math.floor((now - lastMatch) / (1e3 * 60 * 60 * 24));
    ...
    core19.warning(`Rule decay failed: ${e instanceof Error ? e.message : String(e)}`);
    return { rules, decayed };
  } finally {
    db?.close();
  }
}
function getCategoryDecayStats(db, repo) {
  const query = db.prepare(`
    SELECT category,
      COUNT(*) as total,
...
function runRuleEngine(files, customRules, discoveredRules) {
  const findings = [];
  const allRules = [...customRules, ...discoveredRules].filter((r) => r.enabled && r.confidence >= 30);
  for (const rule of allRules) {
    for (const file of files) {
      if (rule.fileGlob && !minimatch4(file.path, rule.fileGlob)) continue;
      if (rule.type === "regex" && rule.pattern) {
        let regex2;
        try {
          regex2 = new RegExp(rule.pattern, "i");
        } catch {
          continue;
        }
        for (const hunk of file.hunks) {
          for (const change of hunk.changes) {
            if (change.type === "add" && regex2.test(change.content)) {
              findings.push({
                file: file.path,
                line: change.line,
      ...
      if ((rule.type === "pattern" || rule.type === "glob") && rule.fileGlob) {
        if (minimatch4(file.path, rule.fileGlob)) {
...
function executeRuleEngine(files, workspace, repo) {
  const customRules = loadCustomRules(workspace);
  core19.info(`Rule engine: ${customRules.length} custom rule(s) loaded`);
  ...
  core19.info(`Rule engine: ${newDiscovered} discovered rule(s) active`);
  ...
  if (decayed > 0) {
    core19.info(`Rule engine: ${decayed} rule(s) decayed`);
  }
  ...
  const rulesSkipped = customRules.filter((r) => !r.enabled).length + decayedRules.filter((r) => !r.enabled).length;
...
var VALID_CATEGORIES = ["security", "compliance", "performance", "bug", "style", "architecture"];
function validateSeverity(s) {
  return VALID_SEVERITIES.includes(s) ? s : "medium";
}
function validateCategory(s) {
  return VALID_CATEGORIES.includes(s) ? s : "bug";
}
function rowToPersistedRule(row) {
  return {
    id: String(row.id),
    ...
    fileGlob: row.file_glob ? String(row.file_glob) : void 0,
...
async function run() {
  try {
    ...
    const token = process.env.GITHUB_TOKEN || core20.getInput("github_token");
    if (!token) {
      core20.setFailed("GITHUB_TOKEN is required");
      return;
    }
    ...
    if (!prNumber) {
      core20.info("No PR number found \xE2\u20AC\u201D skipping review");
      return;
    }
    const owner = ctx.repo.owner;
    const repo = ctx.repo.repo;
    const isManualTrigger = ctx.eventName === "issue_comment";
    core20.info(`Mizumi reviewing ${owner}/${repo}#${prNumber} with ${config.provider}/${config.model}`);
    if (config.dryRun) core20.info("DRY RUN: review will be logged but not posted");
    ...
    if (isManualTrigger) {
      ...
      if (cmd?.command === "describe") {
        core20.info("Running /mizumi describe...");
        ...
        core20.info("Description posted");
        return;
      }
      if (cmd?.command === "improve") {
        ...
        core20.info("Running /mizumi improve...");
        const result = await generateFix(octokit, owner, repo, prNumber, config);
        await octokit.rest.issues.createComment({
          owner,
          repo,
      ...
      if (cmd?.command === "test") {
        core20.info("Running /mizumi test...");
        const diff2 = await fetchDiff(octokit, owner, repo, prNumber, config.excludePatterns);
      ...
      if (cmd?.command === "spend") {
        core20.info("Running /mizumi spend...");
      ...
      if (cmd?.command === "review" && cmd.args) {
        ...
        core20.info("Custom review instructions: " + manualInstructions);
      }
    }
    if (!config.autoReview && !isManualTrigger) {
      core20.info("auto_review is false \xE2\u20AC\u201D skipping. Use /mizumi to trigger.");
      return;
    }
    if (!isManualTrigger && config.autoPauseAfter > 0) {
      ...
      if (reviewCount >= config.autoPauseAfter) {
        core20.info(`Auto-paused: ${reviewCount} reviews already posted (limit=${config.autoPauseAfter}). Use /mizumi to resume.`);
        return;
      }
    }
    if (checkAndMarkDelivery(workspace, deliveryId)) {
      core20.info("Duplicate webhook delivery \xE2\u20AC\u201D skipping");
      return;
    }
    if (!isManualTrigger && checkAndMarkSha(workspace, headSha)) {
      core20.info(`Already reviewed SHA ${headSha.slice(0, 7)} \xE2\u20AC\u201D skipping. Use /mizumi to force.`);
      return;
    }
    if (config.autoFix) {
      try {
        ...
        if (autoFixed > 0) {
          core20.info(`Auto-fixed ${autoFixed} suggestion(s) via \xF0\u0178\u2018\x8D reaction approval`);
          core20.setOutput("auto_fixed", autoFixed);
        }
      } catch (e) {
        core20.warning("Auto-fix processing failed: " + (e instanceof Error ? e.message : String(e)));
      }
    }
    const diff = await fetchDiff(octokit, owner, repo, prNumber, config.excludePatterns);
    core20.info(`Diff: ${diff.files.length} files, +${diff.totalAdditions}/-${diff.totalDeletions}`);
    if (diff.files.length === 0) {
      core20.info("No changed files after exclusions \xE2\u20AC\u201D skipping review");
      return;
    }
    ...
    core20.info(`PR classification: ${prClassification.category} (${prClassification.reason})`);
    ...
    core20.info(`Classification: ${classification.tier} (${classification.reason})`);
    ...
    if (slopResult.isSlop) {
      core20.info(`Slop detected: score=${slopResult.score}, reasons: ${slopResult.reasons.join(", ")}`);
    }
    const lineMap = buildLineMapFromRawDiff(diff.rawDiff);
    const ruleFindings = runRules(diff.files);
    core20.info(`Rules: ${ruleFindings.length} deterministic findings`);
    let engineFindings = [];
    try {
      const engineResult = executeRuleEngine(diff.files, workspace, `${owner}/${repo}`);
      engineFindings = engineResult.findings;
      core20.info(`Rule engine: ${engineResult.findings.length} finding(s), ${engineResult.rulesUsed} rule(s) used, ${engineResult.discoveredNew} discovered, ${engineResult.decayed} decayed`);
    } catch (e) {
      core20.warning(`Rule engine failed: ${e instanceof Error ? e.message : String(e)}`);
    }
    let linterFindings = [];
    try {
      linterFindings = runLinters(workspace, diff.files.map((f) => f.path));
      if (linterFindings.length > 0) core20.info(`Linters: ${linterFindings.length} finding(s)`);
    } catch (e) {
      core20.warning(`Linter scan failed: ${e instanceof Error ? e.message : String(e)}`);
    }
    try {
      const depFindings = runDependencyAudit(workspace);
      if (depFindings.length > 0) {
        linterFindings.push(...depFindings);
        core20.info(`Dependency audit: ${depFindings.length} CVE finding(s)`);
      }
    } catch (e) {
      core20.debug(`Dependency audit skipped: ${e instanceof Error ? e.message : String(e)}`);
    }
...
${skills.loaded}`;
    ...
    if (guarded.truncated) {
      core20.warning(`Diff truncated: ${guarded.estimatedTokens} tokens (exceeds context limit for ${config.provider})`);
    }
...
This PR appears to contain low-quality AI-generated code (score: ${slopResult.score}/100). Reasons: ${slopResult.reasons.join("; ")}. Focus review on structural issues rather than line-by-line quality.`;
    ...
    if (classification.tier !== "light") {
      try {
        core20.info("Running agent context gathering...");
        ...
        core20.warning("Agent context failed: " + (e instanceof Error ? e.message : String(e)));
      }
    }
    core20.info("Running review pass...");
    ...
    core20.info(`First pass: ${review.comments.length} findings, decision=${review.decision} (${reviewUsage.inputTokens + reviewUsage.outputTokens} tokens)`);
    core20.info("Running self-critique pass...");
    await rateLimiter.acquire();
    const filtered = await runCritique(review, config);
    core20.info(`After critique: ${filtered.comments.length} findings (threshold=${config.confidenceThreshold})`);
    const learningWeights = computeLearningWeights(workspace, owner + "/" + repo);
    if (Object.keys(learningWeights).length > 0) {
      core20.info("Learning weights: " + JSON.stringify(learningWeights));
    ...
    try {
      ...
      if (suppressed.size > 0) {
        core20.info(`Adaptive noise: ${suppressed.size} suppressed patterns \xE2\u20AC\u201D ${[...suppressed].join(", ")}`);
        ...
        if (reduced > 0) core20.info(`Adaptive noise: ${reduced} findings confidence-reduced below threshold`);
      }
    } catch {
    }
    ...
    if (config.confidenceCalibration || config.complianceCheck) {
      const calibrationPromise = config.confidenceCalibration ? calibrateConfidence(filtered, config).catch((e) => {
        core20.warning("Calibration failed: " + (e instanceof Error ? e.message : String(e)));
        return null;
      ...
      const compliancePromise = config.complianceCheck ? (async () => {
        try {
          ...
          core20.warning("Compliance check failed: " + (e instanceof Error ? e.message : String(e)));
          return [];
        }
      })() : Promise.resolve([]);
      ...
      if (calibrated) {
        ...
        core20.info("Calibration: " + highCount + " high, " + (calibrated.length - highCount - lowCount) + " medium, " + lowCount + " low");
        filtered.comments = calibrated;
      }
      complianceResults = compliance;
      if (complianceResults.length > 0) {
        core20.info("Compliance: " + complianceResults.length + " issue(s) checked");
      }
    }
    ...
    if (deletedCount > 0) core20.info(`Cleaned up ${deletedCount} outdated comment(s)`);
    if (config.dryRun) {
      core20.info("DRY RUN: Skipping review post. Findings:");
      for (const c of mergedReview.comments) {
        core20.info(`  [${c.severity}] ${c.file}:${c.line} \xE2\u20AC\u201D ${c.category}: ${c.message.slice(0, 200)}`);
      }
      core20.setOutput("review_id", 0);
      core20.setOutput("finding_count", mergedReview.comments.length);
      core20.setOutput("risk_score", mergedReview.riskScore);
    } else {
      core20.info("Posting review...");
      ...
      core20.info(`Review posted: id=${result.reviewId}, findings=${result.findingCount}, risk=${result.riskScore}`);
      core20.setOutput("review_id", result.reviewId);
      core20.setOutput("finding_count", result.findingCount);
      core20.setOutput("risk_score", result.riskScore);
      if (complianceResults.length > 0) {
        const topCompliance = complianceResults[0].compliance;
        core20.setOutput("compliance", topCompliance);
    ...
    if (config.gateThreshold !== "none" && !config.dryRun) {
      try {
        ...
        core20.info(`Merge gate: ${gateResult} (threshold=${config.gateThreshold})`);
      } catch (e) {
        core20.warning("Gate status post failed: " + (e instanceof Error ? e.message : String(e)));
      }
    }
...
*Threshold: ${config.spendThreshold.toLocaleString()} tokens \xE2\u20AC\u201D this review used ${spendEntry.totalTokens.toLocaleString()} tokens.*

---
*Posted by Mizumi*`;
        await createOrUpdateSpendComment(octokit, owner, repo, prNumber, dashboardBody);
        core20.info(`Spend dashboard posted: ${spendEntry.totalTokens} tokens exceeded threshold of ${config.spendThreshold}`);
      } catch (e) {
        core20.warning("Spend dashboard comment failed: " + (e instanceof Error ? e.message : String(e)));
      }
    }
    recordFindings(
      workspace,
    ...
    const memoryUpdate = filtered.comments.filter((c) => c.severity === "critical" || c.severity === "high").map((c) => `- [${c.severity}] ${c.file}:${c.line} \xE2\u20AC\u201D ${c.category}: ${c.message}`).join("\n");
    for (const c of mergedReview.comments) {
    ...
    if (generatedSkills.length > 0) core20.info(`Auto-generated ${generatedSkills.length} skill(s)`);
    try {
      ...
      if (persistResult.committed) {
        core20.info("Learning data persisted: " + persistResult.filesPushed + " file(s), sha=" + persistResult.commitSha);
      }
    } catch (e) {
      core20.warning("Learning persistence failed: " + (e instanceof Error ? e.message : String(e)));
    }
    core20.info("Mizumi review complete");
  } catch (error2) {
    core20.error(`Mizumi error: ${error2 instanceof Error ? error2.stack || error2.message : String(error2)}`);
    core20.setOutput("review_id", 0);
    core20.setOutput("finding_count", 0);
    core20.setOutput("risk_score", -1);
  }
}
